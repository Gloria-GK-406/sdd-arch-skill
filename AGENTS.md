...
Per-runtime manifests must stay in sync when versioning or renaming: `.Codex-plugin/` (plugin + marketplace), `.codex-plugin/`, `.cursor-plugin/`, `.kimi-plugin/`, plus `package.json`. The canonical repo slug referenced in docs/manifests is `Gloria-GK-406/sdd-arch-skill`. The `skills/` directory is the unit that gets installed (via `npx skills`, native plugin marketplace, or copying into the agent's skills dir).
