...
- Invoke it **once at spec finalization** (touchpoint A) and **once at branch wrap-up before merge** (touchpoint B).
...
### Touchpoint B — at branch wrap-up (code landed, before merge)
...
- [ ] Touchpoint B ran at branch wrap-up: `arch-spec-review` dispatched **with the spec document** as material; findings resolved
...
- **Host flow**: this skill is an **add-on** to the project's existing spec-driven flow (e.g. `superpowers:writing-plans` / `superpowers:executing-plans` / `superpowers:finishing-a-development-branch`); it does not replace it.
