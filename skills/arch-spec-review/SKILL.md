...
- If no relevant architecture documents are supplied or found for the changed footprint, there is no recorded intent to review against. **Report that architecture review cannot be performed for that footprint and stop.** Do not route, bootstrap, reconcile, or mention running reconcile; the orchestrator owns that decision.
...
No relevant architecture documents supplied/found → stop with a no-review report only; do not route, bootstrap, reconcile, or mention reconcile.
...
You hold code to intent; you change nothing and decide nothing. When documents exist, output is a compliance report (conflict artifacts + reconciliation signals) handed to arch-doc-reconcile / the human.
```

## Workflow

1. **Relevant-document check**. Determine the architecture documents supplied for, or directly discoverable for, the changed footprint. If the set is empty, **stop with a no-review report**: no relevant architecture documents were supplied/found, so architecture review cannot be performed for that footprint. Do not call this drift, do not bootstrap documents, do not route to another skill, and do not mention reconcile.
2. **Completeness gate (precondition)**. Determine the **consumed** documents: the directly matched ones plus every document whose invariants/content you actually use after expanding along the component graph. Run the gate's three machine checks (`intent-contract` "Completeness gate") on each. **Any `⏳` slot in a consumed document → HALT**: return "run `arch-why-elicit` to complete it, then resubmit". Do not review a partial map. (Documents merely passed over by a pointer, not consumed, do not trigger this.)
3. **Footprint + blast radius** (per `intent-contract` §6). For each changed file: attribute it to its **owning component** (coarse directory attribution, no mapping table); traverse the document graph's component edges (§2 seams, §5 drill-downs, preamble parent/child) for candidate radius; **calibrate by marked-boundary crossing** — if the change is component-internal and crosses no `🧱/💡/🚧`, radius is **local**; once it touches a marked boundary or a face another component's seam references, radius **expands** to all referencing parties. **Free finding only after at least one relevant document exists**: a changed file whose owning component is outside the documented graph = unregistered component = drift.
4. **Cross-boundary detection** (the core pass). For each changed region, hold it against the in-scope **marked** invariants and the section-level `> **Rationale** <strength>` blocks of the owning and referencing components. A crossing = the code now contradicts or erodes a marked invariant or rationale. Check the rationale's **grounding pointer** too: if it reads "no registry import" and the code now imports the registry, the grounding is now false — that is the crossing made concrete.
5. **Guard-test inversion check**. Scan the **test** changes in the diff: did any test that asserted an invariant's behavior get **flipped to assert the opposite**, weakened, or deleted? A flipped/removed guard test is itself a finding and a strong signal the violation was made to pass CI — tie it to the invariant it used to guard. Green tests on an inverted guard validate nothing.
6. **Reconcile against the spec declaration (only if a spec was provided)**. A crossing the spec **explicitly declares it will make** carries the human's explicit authorization, granted at spec approval — review **trusts the declaration** (the same way the completeness gate trusts the document) and does **not** re-gate the decision. Its only job here is to confirm **intent ↔ landing consistency**. Match each crossing from steps 4–5 by **doc-path + INV-id**:
...
Review's whole output is a **report**, never an edit. It contains either a no-review report, or document-backed review entries:
- **No-review report** when no relevant architecture documents were supplied/found: say architecture review cannot be performed for that footprint and stop there.
...
- [ ] Relevant-document check ran; if no relevant documents were supplied/found, stopped with only a no-review report and did not route, bootstrap, or mention reconcile
- [ ] If relevant documents existed, gate ran; every **consumed** document is `⏳`-free (else halted, did not review)
- [ ] If relevant documents existed, every changed file mapped to an owning component + blast radius; unregistered components flagged as free findings
- [ ] If relevant documents existed, every in-radius **marked** invariant and section rationale held against the code (grounding pointers re-checked)
- [ ] If relevant documents existed, test diff scanned; each flipped/removed guard test reported and tied to its invariant
- [ ] If relevant documents existed and a spec was provided: each crossing classified — declared+consistent (authorized → reconciliation signal, even a 🧱) / declared-but-inconsistent ("A vs B" or wider scope → violation) / undeclared (→ violation); authorized crossings not falsely flagged, inconsistent ones not waved through
- [ ] If relevant documents existed, each violation emitted as a full conflict artifact with a candidate root cause; each compliant-but-document-affecting change emitted as a reconciliation signal
- [ ] If relevant documents existed, no judgment passed in the `⏳` region; no architecture decision taken; no reconciliation routed or performed
- [ ] Nothing modified — review is read-only (no code edit, no document edit)

## Dependencies

`../arch-docs-conventions/references/intent-contract.md` (Completeness gate · §3 markers/invariants · §5 seam/drill-down · §6 footprint/blast-radius · §8 `enforced_by` · the conflict-artifact shape) · the target architecture documents · the landed change's diff · the corresponding code · **optionally** the landed spec document (supplied by `arch-spec-flow`; when present its architecture-impact declaration drives the intended-vs-accidental classification). **Next phase when review consumed documents**: `arch-doc-reconcile` (read-only) consumes the conflict artifacts and reconciliation signals and turns them into **suggested** document changes; a human adopts the suggestions and `arch-doc-update` / `arch-doc-build` carry them out — or a spec redesign carries the artifacts. Review itself routes nothing. A no-doc no-review report has no next phase inside this skill.
